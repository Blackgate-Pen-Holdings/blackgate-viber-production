...
class SetupManager {
	...
	constructor() {
		console.log('🚀 Blackgate Viber Setup');
		console.log('============================\n');
	}
	...
	private async collectUserConfig(): Promise<void> {
		...
		if (useAIGateway) {
			...
			const gatewayName = wranglerConfig.vars?.CLOUDFLARE_AI_GATEWAY || 'balckgate-viber';
	...
	private displayFinalReport(report: ReadinessReport, resources: ResourceInfo): void {
		...
		const hasOpenAI = Object.keys(this.config.devVars).includes('OPENAI_API_KEY');
		...
		if (!hasOpenAI || hasOAuth || !hasRemoteR2 || isARM64) {
			console.log('\n💡 Setup Information:');
